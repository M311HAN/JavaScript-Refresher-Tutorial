...
document.addEventListener('DOMContentLoaded', (event) => {
  document.getElementById('arrayAddButton').addEventListener('click', addToArray);
  document.getElementById('arrayRemoveButton').addEventListener('click', removeFromArray);
...
function getUser() {
  ...
  if (userName) {
    document.getElementById('mapOutput').textContent = `User ID ${userId} is ${userName}.`;
  } else {
    document.getElementById('mapOutput').textContent = `No user found with ID ${userId}.`;
  }
}
...
function removeUser() {
  ...
  if (isDeleted) {
    document.getElementById('mapOutput').textContent = `User with ID ${userId} removed.`;
  } else {
    document.getElementById('mapOutput').textContent = `No user found with ID ${userId} to remove.`;
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('removeUserButton').addEventListener('click', removeUser);
});
...
function checkAnswer(quizType) {
    ...
    const quizDetails = {
        loops: { correctAnswer: "3", message: "Correct! A 'for' loop requires initialization, a condition, and an increment." },
        conditions: { correctAnswer: "2", message: "Correct! 'if (i != 5)' executes code if i is NOT equal to 5." },
        variables: { correctAnswer: "3", message: "Correct! The 'const' keyword declares a variable that cannot be reassigned." },
        arrays: { correctAnswer: "1", message: "Correct! The 'push()' method adds an element to the end of an array." },
        functions: { correctAnswer: "1", message: "Correct! Functions typically use parameters to receive inputs." },
        events: { correctAnswer: "3", message: "Correct! The 'addEventListener()' method is used to attach an event handler to a specified element." },
        maps: { correctAnswer: "2", message: "Correct! The 'get()' method is used to retrieve a value from a Map using a key." },
        jquery: { correctAnswer: "3", message: "Correct! The '.hide()' method is used to hide an element in jQuery." }
    };

    const resultText = document.getElementById('quiz-result');
    if (selectedAnswer === "") {
      // Use a different color to draw attention but keep it friendly
        resultText.textContent = "Whoops! It looks like you forgot to pick an answer. Give it another shot!";
